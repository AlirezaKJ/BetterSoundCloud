...
setInterval(() => {
  ...
  var endduration = document.querySelectorAll(
    ".playbackTimeline__duration span",
  )[1];
  var currentduration = document.querySelectorAll(
    ".playbackTimeline__timePassed span",
  )[1];
  var currentsongtitle = document.querySelector(".playbackSoundBadge__title");
  var currentsongcover = document
    .querySelector(".playControls__soundBadge .image__lightOutline span")
    .getAttribute("style")
    .split(";")[0]
    .split('"')[1]
    .replace("120x120", "500x500");
  var currentsongurl = document
  ...
  var currentsongliked = document.querySelector(
    ".playbackSoundBadge__actions .playbackSoundBadge__like",
  );
  ...
  console.log(
    "BSCReceive|CurSongTitle|" + currentsongtitle.innerText.split("\n")[1],
  );
